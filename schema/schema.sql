...
(
    ...
    stream_sequence_nr    sequence_nr                              NOT NULL,
    UNIQUE (stream_type, stream_id, stream_sequence_nr)
);
...
BEGIN
    SELECT MAX(stream_sequence_nr) + 1 = _expected_stream_sequence_nr
    FROM events
    ...
    INSERT INTO events (event_type, event_id, event_payload, stream_type, stream_id,
                        stream_sequence_nr)
    VALUES (_event_type, _event_id, _event_payload, _stream_type, _stream_id,
            _expected_stream_sequence_nr);
END;
$$ LANGUAGE plpgsql;
